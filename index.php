...
function convertImage($source, $destination, $format) {
    $allowedFormats = ['jpg', 'png', 'bmp', 'gif', 'ico', 'tiff', 'webp', 'pdf'];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    foreach ($_FILES['images']['tmp_name'] as $key => $tmp_name) {
		...
		$allowed_formats = ['jpg', 'png', 'bmp', 'gif', 'ico', 'tiff', 'webp', 'pdf'];
    ...
    $resp .= '<button data-href="download.php?file=' . urlencode($zip_path) . '" id="downloadButton" class="button-class flex flex-wrap item-center p-4 bg-green-500 hover:bg-green-800 text-white rounded-md transform transition duration-500 ease-in-out hover:scale-115 w-1/2">Download ZIP file</button>';
    ob_end_clean();
...
<body class="bg-gradient-to-r from-indigo-800 via-purple-700 to-pink-600 flex justify-center items-center min-h-screen">
    <div class="w-3/4 p-6 bg-gray-600 shadow-md rounded-lg animate__animated animate__slideInRight">
        <!-- Title Section -->
        ...
        <div id="form-div" class="flex flex-col w-3/4 justify-self-center">
            ...
            <div id="button-div" class="flex flex-col items-center mt-8">
                <button type="submit" id="submit" class="w-1/5 bg-blue-500 text-white font-semibold py-2 shadow rounded-md transform transition duration-500 hover:bg-blue-600 hover:scale-110">Convert</button>
            </div>
        </form>
